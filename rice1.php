...
$db_host = 'localhost';
$db_user = 'root';
$db_pass = ''; // Your database password, often empty for 'root' on local setups
...
if ($conn->query($sql_create_analysis_results) === TRUE) {
    echo "Table 'analysis_results' created successfully or already exists.<br>";
} else {
    echo "Error creating table 'analysis_results': " . $conn->error . "<br>";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_analysis_results'])) {
    if (isset($_SESSION['user'])) {
        ...
        // Get analysis data from POST request
        ...
        $stmt = $conn->prepare("INSERT INTO analysis_results (user_id, total_grains, avg_length, avg_width, grain_color, grade, broken_grains, chalky_grains, immature_grains, color_consistency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("iiddsssddd", $userId, $total_grains, $avg_length, $avg_width, $grain_color, $grade, $broken_grains, $chalky_grains, $immature_grains, $color_consistency);

if ($stmt->execute()) {
    echo "Analysis results saved successfully.";
} else {
    echo "Error saving analysis results: " . $stmt->error;
}

        // Bind parameters (s = string, i = integer, d = double)
        // Adjust types based on your actual data and database schema
        $stmt->bind_param("iisdssdddd",
            $userId,
            $total_grains,
            $avg_length,
            $avg_width,
    ...
    // Important: Exit after sending JSON response
    exit;
}
...
<body>
    ...
    <div class="main-content">
      ...
      <div id="farmer-dashboard" class="dashboard-section">
        ...
        <div id="detailed-analysis" style="display:none; margin-top: 3rem;">
          <h3 style="margin-bottom: 1.5rem; color: var(--primary);">Detailed Grain Analysis</h3>
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Comprehensive Grain Metrics</h3>
            </div>
            <div class="charts-container">
              <div class="chart-card">
  ...
  <script>
    ...
    function simulateAnalysisResults() {
      ...
      const avgLength = (5 + Math.random() * 2).toFixed(1);
      const avgWidth = (1.8 + Math.random() * 0.8).toFixed(1);
      ...
      simulateDetailedAnalysisResults();

      // Show success status
      showAnalysisStatus('Analysis completed successfully!', 'success');
    ...
    // Simulate detailed analysis results for the new section
    function simulateDetailedAnalysisResults() {
        const brokenGrains = (Math.random() * 10).toFixed(1);
        const chalkyGrains = (Math.random() * 8).toFixed(1);
        const immatureGrains = (Math.random() * 5).toFixed(1);
        const colorConsistency = (70 + Math.random() * 30).toFixed(1); // 70-100%
    ...
    function drawCharts() {
      ...
      if (scatterCtx) {
        ...
        const scatterData = Array.from({ length: 50 }, () => ({
            x: (5 + Math.random() * 2).toFixed(2), // Length between 5 and 7
            y: (1.8 + Math.random() * 0.8).toFixed(2) // Width between 1.8 and 2.6
        }));
      ...
      if (defectCtx) {
        defectChart = new Chart(defectCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        parseFloat(document.getElementById('broken-grains').textContent),
                        parseFloat(document.getElementById('chalky-grains').textContent),
                        parseFloat(document.getElementById('immature-grains').textContent),
                        (100 - parseFloat(document.getElementById('broken-grains').textContent) - parseFloat(document.getElementById('chalky-grains').textContent) - parseFloat(document.getElementById('immature-grains').textContent)).toFixed(1)
                    ], // Using simulated values for defects
